    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        max_GhostPosition = None
        max_Value = float('-inf')

        for livingGhost in livingGhostPositionDistributions:
            for key, value in livingGhost.items():
                if value >= max_Value:
                    max_GhostPosition = key
                    max_Value = value
        ...
        for action in legal:
            ...
            if min_Distance > self.distancer.getDistance(succesorPosition, max_GhostPosition):
                min_Distance = self.distancer.getDistance(succesorPosition, max_GhostPosition)
                best_Action = action
